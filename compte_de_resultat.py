...
def excel(annee_fiscale):
    ...
    number_of_months = (end_date.year - starting_date.year) * 12 + end_date.month - starting_date.month + 1
        ...
        if is_subcategory(value):
            ...
            for subcategory_name, account_list in subcategories.iteritems():
                ...
                for (_, account_number) in account_list:
                    ...
                    result[category_name]['detail'][subcategory_name]['detail'][account_number] = [0] * number_of_months
    ...
    for month_index in months:
        ...
        for key, value in tableau.iteritems():
            if is_subcategory(value):
                ...
                for subcategory_name, account_list in subcategories.iteritems():
                    for (sign, account_number) in account_list:
                        make_account_list_totals(
                            ...
                            result[category_name]['detail'][subcategory_name]['detail'][account_number],
                ...
                account_list = value
                for (sign, account_number) in account_list:
                    make_account_list_totals(operator_map[sign], account_number, financial_lines, month_index,
                        result[category_name]['total'], result[category_name]['detail'][account_number])
            elif is_total_list(value):
                total_name = key
                category_list = value
                for (sign, category_name) in category_list:
                    make_total_list_totals(operator_map[sign], category_name, category_list, result, month_index, result[total_name]['total'])

    unused_accounts = [account for account in all_accounts if int(account['Code']) not in used_accounts]

    workbook = xlsxwriter.Workbook('compte_de_resultat-{}.xlsx'.format(now.strftime('%Y%m%d%H%M%S')))
    ...
    worksheet.merge_range('A1:N1', u'Compte de resultat SIF du %s au %s' % (starting_date.strftime('%m/%Y'), end_date.strftime('%m/%Y')),  title_format)

    worksheet.write(2, 1, u'Intitulé de la ligne'),

    [ worksheet.write(
        2, 2 + month_index, (starting_date + relativedelta(months=month_index)).strftime('%m/%Y'), date_header_format) for month_index in months]
    ...
    for category in result:
        if result[category]['type'] == 'subcategory':
            ...
            for subcategory in result[category]['detail']:
                worksheet.write(row, col + 1, u'{}'.format(subcategory), subcategory_format)
                row +=1
                for account_number in result[category]['detail'][subcategory]['detail']:
                    ...
                    worksheet.write(row, col + 1, u'{}'.format(account_description).lower().capitalize())
                    account_total = result[category]['detail'][subcategory]['detail'][account_number]
                    [ worksheet.write(row, col+2 + index, total, money_format) for index, total in enumerate(account_total) ]
                    row += 1
                subcategory_total = result[category]['detail'][subcategory]['total']
                worksheet.write(row, col + 1, u'Total %s' % subcategory, subcategory_format)
                [ worksheet.write(row, col+ 2 + index, total, money_format) for index, total in enumerate(subcategory_total) ]
                row += 2
            category_total = result[category]['total']
            worksheet.write(row, col + 1, u'TOTAL %s' % category)
            [ worksheet.write(row, col+ 2 + index, total, money_format) for index, total in enumerate(category_total) ]
            row += 2
            ...
            for account_number in result[category]['detail']:
                ...
                worksheet.write(row, col, account_number)
                worksheet.write(row, col +1 , u'{}'.format(account_description.lower().capitalize()))
                [ worksheet.write(row, col+ 2 + index, total, money_format) for index, total in enumerate(value) ]
                row += 1
            category_total = result[category]['total']
            worksheet.write(row, col + 1, u'TOTAL %s' % category)
            [ worksheet.write(row, col+2 + index, total, money_format) for index, total in enumerate(category_total) ]
            ...
            [ worksheet.write(row, col+ 2 + index, total, money_format) for index, total in enumerate(total) ]
            row +=2


    unused_lines_worksheet = workbook.add_worksheet(u'lignes non utilisées ({})'.format(len(unused_lines)))
    row = 0
    col = 0
    unused_lines_worksheet.write_row(row, col,(u'Date', u'Numéro de compte', u'Description du compte', u"Numéro d'écriture", u'Montant'))
    row += 1
    [ unused_lines_worksheet.write_row(row + index, col, (parse_date(line['Date']).strftime("%d/%m/%Y"), line['GLAccountCode'], u'{}'.format(line['GLAccountDescription']), line['EntryNumber'], line['AmountDC']))
        for index, line in enumerate(unused_lines) ]

    unused_accounts_worksheet = workbook.add_worksheet(u'Comptes non utilisés ({})'.format(len(unused_accounts)))
    row = 0
    col = 0
    ...
    row += 1
    [ unused_accounts_worksheet.write_row(row + index, col,( account['Code'], u'{}'.format(account['Description'])))
...
def parse_date(date):
    return datetime.datetime.fromtimestamp(float(re.search('/Date\((.*)\)/', date).group(1))/1000)
...
def find_account_total(account_number, all_financial_lines):
        account_total = sum(line['AmountDC'] for line in all_financial_lines if line['GLAccountCode'] == str(account_number))
